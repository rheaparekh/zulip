    ...
    for message in all_messages:
        ...
        if 'subtype' in message.keys():
            ...
            # For attachments
                ...
                uploads_list = build_uploads(added_users[user], realm_id, file_user_email,
                                             fileinfo, s3_path, uploads_list)

                attachment_id = attachment_id_list[attachment_id_count]
                zerver_attachment = build_zerver_attachment(realm_id, message_id, attachment_id,
...
def build_uploads(user_id: int, realm_id: int, email: str, fileinfo: ZerverFieldsT, s3_path: str,
                  uploads_list: List[ZerverFieldsT]) -> List[ZerverFieldsT]:
    upload = dict(
...
def build_zerver_attachment(realm_id: int, message_id: int, attachment_id: int,
                            ...
                            zerver_attachment: List[ZerverFieldsT]) -> List[ZerverFieldsT]:
    attachment = dict(
        owner=user_id,
        messages=[message_id],
        id=attachment_id,
...
def do_convert_data(slack_zip_file: str, realm_subdomain: str, output_dir: str, token: str) -> None:
    ...
    os.makedirs(uploads_folder + str(realm_id), exist_ok=True)
...
def process_uploads(upload_list: List[ZerverFieldsT], upload_dir: str) -> List[ZerverFieldsT]:
    ...
    for upload in upload_list:
        ...
        upload_path = ('%s%s' % (upload_dir, upload_s3_path))
